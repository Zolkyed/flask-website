...
class Database:
    ...
    def search_animal(self, terme):
        cursor = self.get_connection().cursor()
        search_terme = '%' + terme.lower() + '%'
        query = ("SELECT id, nom, espece, race, age, description, "
                "courriel, adresse, ville, cp FROM animaux "
                "WHERE nom LIKE ? OR espece LIKE ? OR race LIKE ? OR "
                "description LIKE ? OR courriel LIKE ? OR "
                "ville LIKE ?")
        cursor.execute(query, (search_terme,) * 6)
        all_data = cursor.fetchall()
        return [_build_animal(item) for item in all_data]
